...
# time_steps = 4
time_steps_train = 4
# time_steps_eval_list = [10, 100, 1000]
# time_steps_eval_list = [10, 100, 200]
time_steps_eval_list = [10, 100]
...
def sign_grad(op, grad): 
    ...
    # []Why would you want a gradient of zero if input is outside of [-1,1]?
    # []Sonst wird grad einfach durchgereicht, wa?
...
y_eval_list = []
for time_steps in time_steps_eval_list:
...
for time_steps in time_steps_eval_list:
    ...
    eval_batch_generator_list.append(eval_batch_generator)
...
# XXX Maybe simply compute the probability of the counter event here?
...
with tf.Session() as sess:
    ...
    try:
        for i in range(epochs):
            ...
            # - on validation set
            eval_accuracy_list_ = []
            for eval_index, eval_batch_generator in enumerate(eval_batch_generator_list):
                xs_eval, ys_eval = eval_batch_generator.next()

                eval_accuracy_, _, _ = sess.run(
                    [eval_accuracy_list[eval_index], labels, predictions], 
                    feed_dict = {
                        X : xs_eval,
                        y_target : ys_eval
                    }
                ...
                eval_accuracy_list_.append(eval_accuracy_)
            ...
            print('[{}] loss : {}, train accuracy: {}, eval accuracy (digits, acc): {}'\
                .format(
                    i,
                    # train_loss / 1000
                    train_loss,
                    train_accuracy_,
                    # eval_accuracy_
                    zip(time_steps_eval_list, eval_accuracy_list_)
                )
            )
