...
with tf.variable_scope('model') as scope:
    ...
    # Recurrent *non*-interlayer weights
    W = [tf.get_variable(u'W_{}'.format(i),
        # Still needs to be a matrix huh
        shape=[num_nodes_per_layer[i], num_nodes_per_layer[i]],
        initializer=tf.contrib.layers.xavier_initializer()
...
def build_recurrent_network(batch_size, time_steps):
    ...
    for t in range(time_steps):
        ...
        for i in range(1, num_layers):
            ...
            # Potentially don't need to declare it as a tf.zeros anymore
            # for standard RNN
            ...
            with tf.variable_scope(scope, reuse=True):
                W = tf.get_variable('W_{}'.format(i))

            # XXX Do I need to worry about batch_size dimension in matmul, add?
            recurrent_term += tf.matmul(states[i], W)
...
def generate_batches(data, time_steps, batch_size):
    while True:
        ...
        for x in range(batch_size):
            # XXX Here is a bug: len(data) doesn't give us 
            # the total number of samples for a given seq len
            # print('data.shape', data.shape)
